...
const upcomingSessions = sessions.filter(session => 
  ...
  const handleGenerateDietPlan = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/api/diet/generate-diet", dietForm, {
        headers: {
      ...
      console.log("Diet generation response:", response.data);

      if (!response.data || !response.data.success) {
        throw new Error(response.data?.error || "Failed to generate diet plan");
      }

      const dietData = response.data.data;
      if (!dietData) {
        throw new Error("No diet data returned by server");
      }

      console.log("Setting diet plan:", {
        nutrition: dietData.nutrition,
        meals: dietData.meals,
        totals: dietData.totals,
        mealTargets: dietData.mealTargets,
      });

      setDietPlan({
        nutrition: dietData.nutrition || { calories: 0, protein: 0, carbs: 0, fat: 0 },
        meals: dietData.meals || {},
        totals: dietData.totals || { totalCalories: 0, totalProtein: 0, totalCarbs: 0, totalFat: 0 },
        mealTargets: dietData.mealTargets || {},
        dietPlan: dietData.meals || {}, // Keep for backward compatibility with display logic
      });
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {Object.entries(dietPlan?.dietPlan || {}).map(([meal, foods]) => {
                    const getMealIcon = (mealType) => {
                      switch (mealType) {
                        case 'breakfast': return <Coffee className="h-6 w-6 text-orange-500" />;
                        case 'lunch': return <Sun className="h-6 w-6 text-yellow-500" />;
                        case 'dinner': return <Moon className="h-6 w-6 text-indigo-500" />;
                        case 'snacks': return <Cookie className="h-6 w-6 text-pink-500" />;
                        default: return <Apple className="h-6 w-6 text-green-500" />;
                      }
                    };
                    
                    const getMealColor = (mealType) => {
                      switch (mealType) {
                        case 'breakfast': return 'border-orange-200 bg-orange-50';
                        case 'lunch': return 'border-yellow-200 bg-yellow-50';
                        case 'dinner': return 'border-indigo-200 bg-indigo-50';
                        case 'snacks': return 'border-pink-200 bg-pink-50';
                        default: return 'border-green-200 bg-green-50';
                      }
                    };

                    return (
                      <div key={meal} className={`border-2 rounded-xl p-4 ${getMealColor(meal)} hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1`}>
                        <div className="flex items-center mb-4">
                          {getMealIcon(meal)}
                          <h4 className="text-lg font-semibold text-gray-900 ml-2 capitalize">{meal}</h4>
                        </div>
                        <div className="space-y-3">
                          {Array.isArray(foods) && foods.length > 0 ? (
                            <>
                              {foods.slice(0, 5).map((food, index) => (
                                <div key={index} className="bg-white rounded-lg p-3 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                  <div className="font-medium text-gray-900 text-sm mb-1">{food.name}</div>
                                  {food.description && (
                                    <div className="text-xs text-gray-500 mb-2 italic">{food.description}</div>
                                  )}
                              ...
                              {/* Per-meal totals */}
                              <div className="bg-white rounded-lg p-3 shadow-sm border-2 border-gray-200 mt-4">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                  ...
                                  <div className="flex justify-between">
                                    ...
                                    <span className="font-bold text-green-600">
                                      {foods.reduce((sum, f) => sum + f.carbs, 0).toFixed(1)}g
                                    </span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-gray-600">Fat:</span>
                                    <span className="font-bold text-purple-600">
                                      {foods.reduce((sum, f) => sum + f.fat, 0).toFixed(1)}g
                                    </span>
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="text-center text-sm text-gray-500 py-6 border border-dashed border-gray-200 rounded-lg">
                              No food items found for {meal}. Try a different diet type or goal.
                            </div>
                          )}
                        </div>
                      </div>
  ...
  <div className="bg-white rounded-xl shadow-lg p-6">
    ...
    <div className="space-y-6">
      ...
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 z-40 flex items-center justify-center group"
        title="Generate Diet Plan"
      >
        <Apple className="h-6 w-6 group-hover:scale-110 transition-transform duration-200" />
      </button>
      ...
      {showDietModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity duration-300">
          <div className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-100 opacity-100">
            <div className="p-6 border-b border-gray-200">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-bold text-gray-900">Generate Diet Plan</h3>
                <button
            ...
            <div className="p-6">
              {dietError && (
                <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-red-700 text-sm">{dietError}</p>
                </div>
              )}
              
              <form onSubmit={handleGenerateDietPlan} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                <div className="flex space-x-4 pt-4">
                  ...
                  <button
                    type="submit"
                    className="flex-1 px-6 py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700 transition-all duration-200 font-medium shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105"
                    disabled={dietLoading}
                  >
                    {dietLoading ? (
